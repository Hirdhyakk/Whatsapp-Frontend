...
import "./ChatWindow.css"; // Add custom styles as needed.
import ProfileHeader from "../ProfileHeader/ProfileHeader";
import MessageArea from "../MessageArea/MessageArea";
import MessageInput from "../MessageInput/MessageInput";

const ChatWindow = ({ currentChat }) => {
  const [chat, setChat] = useState(currentChat); // Assuming currentChat is passed as prop
  ...
  return (
    <div className="w-100 vh-100 d-flex flex-column bg-dark text-light">
      {chat ? (
        <>
          <ProfileHeader currentChat={chat} />
          <MessageArea messages={chat.messages} isGroup={chat.type === "group"} />
          <MessageInput onSendMessage={handleSendMessage} />
        </>
      ) : (
        <>
          <div className="d-flex flex-column justify-content-center align-items-center vh-100" style={{ backgroundColor: "#212e36" }}>
            <div className="align-content-center text-center m-5">
              <img className="mb-4" width="320px" src="https://static.whatsapp.net/rsrc.php/v3/y6/r/wa669aeJeom.png" alt="" />
              <h2 className="fw-normal mb-4">Download WhatsApp for Windows</h2>
              <p className="mb-4 text-secondary">Make calls, share your screen and get a faster experience when you download the<br /> Windows app.</p>
              <button className="get-whatsapp-btn rounded-pill text-black px-4 py-2 fw-semibold">Get from Microsoft Store</button>
              <div className="mt-5 text-center encrypted-text text-secondary">
